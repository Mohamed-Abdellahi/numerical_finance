...
Compatible with both BasicMapping (pseudo-random MC) and
QMCMapping (quasi-random MC) as the underlying process.
"""
...
class AntitheticMapping(MCMapping):
    ...
    def _build_antithetic(self, process: RandomProcess) -> RandomProcess:
        ...
        if isinstance(process, BSMilstein1D):
            return BSMilstein1D(anti_gen, process._spot,
                                process._rate, process._vol)
        else:
            corr = recover_correlation(process._cholesky)
            return BSMilsteinND(anti_gen, process._spots,
                                process._rates, process._vols, corr)
